...
import { X, Mail, Phone, MapPin, Code2, Edit2, Save, Upload, Trash2, Plus, Link } from 'lucide-react';
...
export default function DeveloperModal({ isOpen, onClose, isDark, isAdmin }: DeveloperModalProps) {
  ...
  const initialData = {
    ...
    about: `Ezequiel John B. Crisostomo is a passionate full-stack developer and system architect dedicated to creating innovative digital solutions that empower organizations and communities. As the lead developer of the YSP Tagum Chapter Web Application, he combines technical expertise with a deep understanding of community service to build tools that enhance organizational efficiency and member engagement.

With a strong foundation in modern web technologies including React, TypeScript, and cloud platforms, Ezequiel specializes in developing scalable, user-friendly applications that solve real-world problems. His work on this platform demonstrates his commitment to leveraging technology for social good.`,
    background: `Born and raised with a passion for technology and community service, Ezequiel pursued a career in software development with the goal of making meaningful contributions to society. His journey in web development began with a curiosity about how digital tools could transform organizational operations and improve service delivery.

Currently serving as the Membership and Internal Affairs Officer at YSP Tagum Chapter, he bridges the gap between technology and community service. This dual role gives him unique insights into the challenges faced by volunteer organizations and informs his approach to developing practical, impactful solutions.`,
    philosophy: `"Technology should serve humanity. Every line of code I write is an opportunity to make someone's life easier, an organization more effective, or a community more connected. Great software is not just about elegant algorithms—it's about understanding people's needs and creating solutions that truly make a difference."`,
    socialLinks: [
      { url: 'https://www.facebook.com/YSPTagumChapter' },
      { url: 'https://github.com/ezequieljohn' }
    ],
    contact: {
    ...
    affiliations: [
      { name: 'Youth Service Philippines', category: 'Organization' },
      { name: 'Tagum City LGU', category: 'Government' },
      { name: 'DepEd Region XI', category: 'Education' },
      { name: 'DSWD Region XI', category: 'Social Welfare' }
    ]
  };

  const [developerData, setDeveloperData] = useState(initialData);
  const [savedData, setSavedData] = useState(initialData);

  if (!isOpen) return null;

  const handleSave = () => {
    setSavedData(developerData);
    setIsEditing(false);
    toast.success('Developer profile updated successfully');
  };

  const handleCancelEdit = () => {
    setDeveloperData(savedData);
    setIsEditing(false);
  ...
  const addSocialLink = () => {
    setDeveloperData({ 
      ...developerData, 
      socialLinks: [...developerData.socialLinks, { url: '' }] 
    });
  };

  const updateSocialLink = (index: number, url: string) => {
    const newSocialLinks = [...developerData.socialLinks];
    newSocialLinks[index] = { url };
    setDeveloperData({ ...developerData, socialLinks: newSocialLinks });
  };

  const removeSocialLink = (index: number) => {
    const newSocialLinks = developerData.socialLinks.filter((_, i) => i !== index);
    setDeveloperData({ ...developerData, socialLinks: newSocialLinks });
  };
  ...
  const addAffiliation = () => {
    setDeveloperData({ 
      ...developerData, 
      affiliations: [...developerData.affiliations, { name: 'New Affiliation', category: 'Category' }] 
    });
  };

  const updateAffiliation = (index: number, field: 'name' | 'category', value: string) => {
    const newAffiliations = [...developerData.affiliations];
    newAffiliations[index][field] = value;
    setDeveloperData({ ...developerData, affiliations: newAffiliations });
  };

  const removeAffiliation = (index: number) => {
    const newAffiliations = developerData.affiliations.filter((_, i) => i !== index);
    setDeveloperData({ ...developerData, affiliations: newAffiliations });
  };
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex-1">
              {isEditing ? (
                <>
                  ...
                  <input
                    type="text"
                    value={developerData.nickname}
                    onChange={(e) => setDeveloperData({ ...developerData, nickname: e.target.value })}
                    ...
                    placeholder="Nickname (e.g., a.k.a Ziek)"
                  />
                  <input
                    type="text"
                ...
                <>
                  ...
                  <p
                    className="text-[#3b82f6] mb-2 italic"
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {developerData.affiliations.map((affiliation, index) => (
                <div
                  key={index}
                ...
                >
                  {isEditing ? (
                    <div className="space-y-2">
                      ...
                      <input
                        type="text"
                        value={affiliation.category}
                        onChange={(e) => updateAffiliation(index, 'category', e.target.value)}
