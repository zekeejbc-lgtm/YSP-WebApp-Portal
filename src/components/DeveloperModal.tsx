...
export default function DeveloperModal({ 
  ...
  const emptyData = {
    ...
    affiliations: [] as { name: string; category: string }[]
  };

  const [developerData, setDeveloperData] = useState(emptyData);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.log('[DevModal] Raw data from backend:', data);
      ...
      const transformedData = {
        ...
        philosophy: data.devPhilosophy || '',
        socialLinks: data.socialLinks?.map(link => ({ url: link.url })) || [],
        contact: {
          email: data.email || '',
          phone: data.phone || '',
        ...
        affiliations: data.affiliations?.map(aff => ({ 
          name: aff.orgName, 
          category: aff.position 
        })) || []
      };
      
      setDeveloperData(transformedData);
      setSavedData(transformedData);
      setImagePreview(transformedData.profileImage); // Set initial image preview from backend
      
      console.log('[DevModal] Transformed data:', transformedData);
      console.log('[DevModal] Profile image URL:', transformedData.profileImage);
      
      updateToastRef.current(toastId, {
        status: 'success',
        progress: 100,
        title: 'Profile Loaded',
        message: 'Developer info loaded successfully',
      });
    } catch (error) {
      console.error('[DevModal] Error fetching data:', error);
      updateToastRef.current(toastId, {
        status: 'error',
        progress: 100,
        title: 'Load Failed',
  ...
  const handleSave = async () => {
    ...
    addUploadToast({
      id: toastId,
      title: 'Saving Developer Profile',
    ...
    try {
      let finalProfileUrl = developerData.profileImage;
      ...
      if (pendingImageFile) {
        ...
        if (uploadResult.success && uploadResult.imageUrl) {
          ...
          console.log('[DevModal] Image uploaded successfully:', finalProfileUrl);
          ...
          setIsSaving(false);
          return; // Stop save if image upload failed
        }
      } else {
      ...
      const backendData: Partial<DevInfoContent> = {
        profileUrl: finalProfileUrl, // Include the profile URL in the update
        name: developerData.name,
        nickname: developerData.nickname,
        organization: developerData.organization,
        position: developerData.position,
        about: developerData.about,
        background: developerData.background,
        devPhilosophy: developerData.philosophy,
        email: developerData.contact.email,
        phone: developerData.contact.phone,
        location: developerData.contact.location,
        affiliations: developerData.affiliations.map((aff, index) => ({
          id: index + 1,
          orgName: aff.name,
          position: aff.category
        })),
        socialLinks: developerData.socialLinks.map((link, index) => ({
          id: index + 1,
          url: link.url
        }))
      };
      ...
      if (success) {
        ...
        const updatedData = { ...developerData, profileImage: finalProfileUrl };
        setDeveloperData(updatedData);
        ...
        updateUploadToast(toastId, {
          ...
          progress: 100,
          title: 'Profile Saved!',
          message: 'Developer profile saved to backend successfully',
        });
        console.log('[DevModal] Save successful, final profile URL:', finalProfileUrl);
      } else {
        updateUploadToast(toastId, {
          status: 'error',
          progress: 100,
  ...
  const handleCancelEdit = () => {
    ...
    setImagePreview(savedData.profileImage); // Reset image preview to saved version
    setPendingImageFile(null); // Clear any pending image
    setIsEditing(false);
  };

  // Handle image selection - just preview locally, don't upload yet
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      console.log('[DevModal] Image preview created locally (not uploaded yet)');
    };
      ...
      >
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {isAdmin && (
              <button
                ...
                disabled={isSaving}
                className="px-4 py-2 rounded-lg transition-all text-sm font-semibold flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                style={{
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center p-6" style={{ minHeight: '500px' }}>
            ...
            <p className="mt-4 text-gray-600 dark:text-gray-400">Loading developer profile...</p>
          </div>
        ) : (
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div className="shrink-0">
              {/* Use imagePreview for display - this shows local preview or backend URL */}
              {imagePreview ? (
                <ImageWithFallback
                  src={imagePreview}
                  alt={developerData.name || 'Developer Profile'}
                  className="w-32 h-32 md:w-40 md:h-40 rounded-xl object-cover border-4 border-[#3b82f6] shadow-lg"
                />
              ) : (
                <div className="w-32 h-32 md:w-40 md:h-40 rounded-xl border-4 border-[#3b82f6] shadow-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
