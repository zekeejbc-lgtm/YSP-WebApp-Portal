...
function createIssuance(data) {
  ...
  if (data.recipients && data.recipients.length > 0) {
    addRecipients(id, data.recipients);
  }
...
function updateIssuance(data) {
  ...
  for (let i = 1; i < values.length; i++) {
    if (values[i][0] === data.id) {
      ...
      if (data.title) values[i][colMap['Title']] = data.title;
      if (data.status) values[i][colMap['Status']] = data.status;
      if (data.emailTitle) values[i][colMap['EmailTitle']] = data.emailTitle;
      if (data.emailMessage) values[i][colMap['EmailMessage']] = data.emailMessage;
      if (data.fieldInputs) values[i][colMap['FieldInputs']] = JSON.stringify(data.fieldInputs);
      if (data.customTemplateUrl !== undefined) values[i][colMap['CustomTemplateUrl']] = data.customTemplateUrl;
      if (data.notes !== undefined) values[i][colMap['Notes']] = data.notes;
 ...
 */
function addRecipients(issuanceId, recipients) {
  const ss = SpreadsheetApp.openById(ISSUANCE_CONFIG.SPREADSHEET_ID);
